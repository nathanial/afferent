...
def test_tokenize_simple : TestCase := {
  name := "tokenize splits words and spaces"
  run := do
    let tokens := Afferent.Widget.tokenize "hello world"
    shouldBe tokens.size 3  -- "hello", space, "world"
    match tokens[0]? with
    | some (Afferent.Widget.Token.word w) => shouldBe w "hello"
    | _ => throw <| IO.userError "Expected word token"
    match tokens[1]? with
    | some Afferent.Widget.Token.space => pure ()
    | _ => throw <| IO.userError "Expected space token"
    match tokens[2]? with
    | some (Afferent.Widget.Token.word w) => shouldBe w "world"
    | _ => throw <| IO.userError "Expected word token"
}

def test_tokenize_newlines : TestCase := {
  name := "tokenize handles newlines"
  run := do
    let tokens := Afferent.Widget.tokenize "line1\nline2"
    shouldBe tokens.size 3  -- "line1", newline, "line2"
    match tokens[1]? with
    | some Afferent.Widget.Token.newline => pure ()
    | _ => throw <| IO.userError "Expected newline token"
}

def test_tokenize_empty : TestCase := {
  name := "tokenize handles empty string"
  run := do
    let tokens := Afferent.Widget.tokenize ""
    shouldBe tokens.size 0
}
...
def test_intrinsic_size_spacer : TestCase := {
  name := "intrinsic size of spacer matches dimensions"
  run := do
...
def test_intrinsic_size_rect : TestCase := {
  name := "intrinsic size of rect uses min dimensions"
  run := do
...
def test_intrinsic_size_rect_no_min : TestCase := {
  name := "intrinsic size of rect without min is zero"
  run := do
    let widget : W := .rect 0 {}
    let (w, h) ← Afferent.Widget.intrinsicSize widget
...
def test_intrinsic_size_row : TestCase := {
  name := "intrinsic size of row sums widths"
  run := do
...
def test_intrinsic_size_column : TestCase := {
  name := "intrinsic size of column sums heights"
  run := do
    let child1 : W := .spacer 1 50 30
    let child2 : W := .spacer 2 60 40
...
def test_intrinsic_size_with_padding : TestCase := {
  name := "intrinsic size includes padding"
  run := do
...
def test_intrinsic_size_nested : TestCase := {
  name := "intrinsic size works for nested containers"
  run := do
...
def test_widget_ids_unique : TestCase := {
  name := "widget builder generates unique IDs"
  run := do
    ...
    shouldBe ids.size uniqueIds.length
}

def test_widget_count : TestCase := {
  name := "widget count includes all nodes"
  run := do
    let widget := Afferent.Widget.build do
      Afferent.Widget.column (gap := 0) {} #[
    ...
    shouldBe widget.widgetCount 6
}

def test_build_column : TestCase := {
  name := "column builder creates vertical flex"
  run := do
    let widget := Afferent.Widget.build do
...
def test_build_row : TestCase := {
  name := "row builder creates horizontal flex"
  run := do
    let widget := Afferent.Widget.build do
...
def test_build_coloredBox : TestCase := {
  name := "coloredBox creates rect with dimensions"
  run := do
    let widget := Afferent.Widget.build do
      Afferent.Widget.coloredBox Color.red 80 60
    match widget with
    | .rect _ style =>
      shouldBe style.backgroundColor (some Color.red)
      shouldBe style.minWidth (some 80)
      shouldBe style.minHeight (some 60)
    | _ => throw <| IO.userError "Expected rect widget"
}

def test_build_card : TestCase := {
  name := "card creates styled flex container"
  run := do
    let widget := Afferent.Widget.build do
    ...
    | .flex _ _ style children =>
      shouldBe style.backgroundColor (some (Color.gray 0.3))
...
def test_measure_spacer : TestCase := {
  name := "measureWidget preserves spacer dimensions"
  run := do
    let widget : W := .spacer 0 100 50
...
def test_measure_rect : TestCase := {
  name := "measureWidget uses rect min dimensions"
  run := do
    let widget : W := .rect 0 { minWidth := some 80, minHeight := some 60 }
    let result ← Afferent.Widget.measureWidget widget 1000 1000
    match result.node.content with
...
def test_measure_flex_children : TestCase := {
  name := "measureWidget recursively measures children"
  run := do
    let child1 : W := .spacer 1 50 30
    let child2 : W := .spacer 2 60 40
    ...
    | .flex _ _ _ children => shouldBe children.size 2
    | _ => throw <| IO.userError "Expected flex widget"
    -- Check LayoutNode has children
    shouldBe result.node.children.size 2
}

/-! ## Grid Tests -/

def test_intrinsic_size_grid : TestCase := {
  name := "intrinsic size of grid computes rows and columns"
  run := do
...
def allTests : List TestCase := [
  ...
  test_tokenize_empty,
  -- Intrinsic size
  test_intrinsic_size_spacer,
  test_intrinsic_size_rect,
  test_intrinsic_size_rect_no_min,
  test_intrinsic_size_row,
  test_intrinsic_size_column,
  test_intrinsic_size_with_padding,
  test_intrinsic_size_nested,
  test_intrinsic_size_grid,
  -- Widget builder
  test_widget_ids_unique,
  test_widget_count,
  test_build_column,
  test_build_row,
  test_build_coloredBox,
  test_build_card,
  -- Measurement
  test_measure_spacer,
  test_measure_rect,
  test_measure_flex_children
]

def runAllTests : IO UInt32 :=
  runTests "Widget Tests" allTests
