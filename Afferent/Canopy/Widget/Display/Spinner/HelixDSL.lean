/-
  Helix Spinner (DSL version) - DNA-like double helix rotating
  This version uses the Shader DSL instead of raw Metal strings.
-/
...
open Afferent.Shader.DSL hiding center size time color  -- Hide to avoid conflict with Arbor
open Linalg
...
def helixShaderDSL : CircleShader := {
  name := "helix"
  instanceCount := 16
  params := [
    ⟨"center", .float2⟩,
...
def helixFragmentDSL : ShaderFragment := helixShaderDSL.compile

/-- Register the helix fragment in the global registry at module load time. -/
initialize helixFragmentDSLRegistration : Unit ← do
  registerFragment helixFragmentDSL

/-- Helix (DSL): DNA-like double helix using GPU shader fragment.
    Passes only 8 floats to GPU; the shader computes all 16 circle positions, sizes, and colors. -/
def helixSpecDSL (t : Float) (color : Color) (dims : Dimensions) : CustomSpec := {
  ...
  collect := fun layout =>
    ...
    RenderM.build do
      RenderM.drawFragment helixFragmentDSL.hash helixFragmentDSL.primitive.toUInt32
        params helixFragmentDSL.instanceCount.toUInt32
  draw := none
}
